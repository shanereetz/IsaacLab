...
# Pre-defined configs
##
from omni.isaac.lab_assets import GOFA_CONFIG  # isort: skip
...
@configclass
class GoFaReachEnvCfg(ReachEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()

        # switch robot to GoFa
        self.scene.robot = GOFA_CONFIG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        # override events
...
@configclass
class GoFaReachEnvCfg_PLAY(GoFaReachEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
